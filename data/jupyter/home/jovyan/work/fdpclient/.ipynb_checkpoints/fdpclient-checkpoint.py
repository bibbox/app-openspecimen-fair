...
class FDPClient:
    ...
    def update(self,type,id, data, format='turtle', **kwargs):
        ...
        Args:
            ...
            **kwargs: Optional arguments that :func:`requests.request` takes.
        """
        logger.debug(f'Update metadata on {self.baseurl}/{type}/{id} with the content: \n{data}')
        update_headers = self.headers.copy()
        update_headers.update({'Content-Type': self.DATA_FORMATS[format]})

        try:
            data = self._check_data(data, format)
            r = requests.put(f'{self.baseurl}/{type}/{id}', data=data, headers=update_headers, **kwargs)
